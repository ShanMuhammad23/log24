  ...
  return (
    <View className="mb-3">
      ...
      <View className="rounded-xl border border-slate-700 bg-slate-900/80 p-3">
        <Text className="font-mono text-sm leading-5 text-slate-100">{value || '—'}</Text>
      </View>
    </View>
  );
}
...
function DecodedField({ label, value }: { label: string; value: unknown }) {
  return (
    <View className="mb-2.5 border-b border-slate-800 pb-2.5 last:mb-0 last:border-b-0 last:pb-0">
      <Text className="text-xs font-semibold uppercase tracking-wide text-blue-300">{label}</Text>
      <Text className="mt-1 font-mono text-sm leading-5 text-slate-100">{formatFieldValue(value)}</Text>
    </View>
  );
}
...
export function MetarReportCard({ report, index }: MetarReportCardProps) {
  ...
  return (
    <View className="mb-4 rounded-2xl border border-slate-700 bg-slate-900 p-4">
      <View className="mb-3 flex-row items-center justify-between">
        <View className="flex-1 pr-2">
          <Text className="text-lg font-bold text-white">{report.name}</Text>
          {report.metarType ? (
            <Text className="mt-0.5 text-xs text-slate-400">{report.metarType}</Text>
          ) : null}
        </View>
        <Pressable
      ...
      {decoded ? (
        <View className="mt-1 rounded-xl border border-slate-700 bg-slate-950/60 p-3">
          <Text className="mb-2 text-sm font-semibold text-slate-300">Decoded fields</Text>
